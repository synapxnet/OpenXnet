...
exports.COMPETITION_TOOL_REGISTRY = exports.COMPETITION_TOOL_NAMES = void 0;
exports.getCompetitionToolDescriptor = getCompetitionToolDescriptor;
exports.parseCompetitionToolArguments = parseCompetitionToolArguments;
/** 竞赛 P0 固定工具名称。 */
exports.COMPETITION_TOOL_NAMES = [
    "aiops.alert.get",
...
exports.COMPETITION_TOOL_REGISTRY = Object.freeze([
    {
...
function getCompetitionToolDescriptor(name) {
    if (typeof name !== "string")
        throw new TypeError("Competition tool name is invalid.");
    const descriptor = exports.COMPETITION_TOOL_REGISTRY.find((candidate) => candidate.name === name);
    if (descriptor === undefined)
        throw new TypeError("Competition tool is not registered.");
    return descriptor;
}
/** 校验工具参数为有界普通 JSON；输入工具和未知参数，返回安全副本，无效时抛出 TypeError。 */
function parseCompetitionToolArguments(descriptor, value) {
    if (!isRecord(value))
        throw new TypeError("Competition tool arguments are invalid.");
    const byteLength = Buffer.byteLength(JSON.stringify(value), "utf8");
    if (byteLength > 64 * 1024)
        throw new TypeError("Competition tool arguments exceed their byte budget.");
    validateArgumentsByTool(descriptor.name, value);
    return JSON.parse(JSON.stringify(value));
}
/** 按固定工具校验精确参数；输入名称和普通对象，无返回，无效时抛出 TypeError。 */
function validateArgumentsByTool(name, value) {
    switch (name) {
...
function requireShape(value, required, optional) {
    ...
    for (const field of required) {
        if (!(field in value))
            throw new TypeError(`Competition tool field '${field}' is required.`);
    }
...
function requireExactAlternative(value, first, second) {
    const keys = Object.keys(value);
    /** 判断当前参数键是否精确匹配候选形状；输入字段列表，返回布尔结果。 */
    const matches = (shape) => keys.length === shape.length && shape.every((field) => field in value);
...
function requireIntegerIfPresent(value, minimum, maximum, required = false) {
...
function requireIntegerArray(value, label, minimum, maximum, minimumItems, maximumItems) {
    if (!Array.isArray(value)
        || value.length < minimumItems
        || value.length > maximumItems
...
function requireEnumText(value, allowed) {
...
function validateVerificationPolicy(value) {
    if (!isRecord(value))
        throw new TypeError("Rollback verification policy is invalid.");
...
//# sourceMappingURL=competition-tool-registry.js.map