...
| 量化模型归因与迭代 | 盘后数据质量/就绪事件、因子贡献、市场漂移、IC 退化、部署和推理探针 | 五年数据集 → 因子流水线 → 30 组并行搜索 → IC/夏普/回撤/对抗验证 → 模型登记 → 1% 模拟盘灰度 → 100% 模拟信号提升 | 数据集、归因门槛、目标修订、发布状态和推理探针 | `synapxnet-quantitative-model-iteration` |
