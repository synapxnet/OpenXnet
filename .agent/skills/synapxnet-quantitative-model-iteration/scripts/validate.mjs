...
const SKILL_NAME = "synapxnet-quantitative-model-iteration";
...
const COMPENSATION_STEP_IDS = Object.freeze(["quantitative-model-rollback"]);
const REQUIRED_TOOLS = Object.freeze([
  "dataops.training.dataset.build", "mlops.feature.pipeline.publish", "mlops.training.search.start",
  "mlops.model.evaluation.run", "mlops.model.register", "mlops.deployment.canary.apply", "mlops.deployment.promote",
]);
...
function validateInput(value) {
  ...
  if (value.schema !== "synapxnet.skill.quantitative-iteration.input.v1" || value.scenarioType !== "quantitative-iteration") throw new Error("Skill input schema or scenario is invalid.");
  ...
  requireTextArray(value.evidenceIds, "evidenceIds", 6);
...
function validateOutput(value) {
  ...
  if (value.schema !== "synapxnet.skill.quantitative-iteration.output.v1" || !["SUCCEEDED", "ROLLBACK_REQUIRED", "HALT"].includes(value.outcome)) throw new Error("Skill output schema or outcome is invalid.");
  ...
  requireTextArray(value.verificationEvidenceIds, "verificationEvidenceIds", value.outcome === "SUCCEEDED" ? 6 : 0, 64);
}
