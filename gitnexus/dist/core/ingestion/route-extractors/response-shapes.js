...
function lastMatchGroup(text, pattern) {
...
function buildShapeResult(successKeys, errKeys) {
    return {
 ...
 */
function findLastExitBoundary(text) {
...
function detectPHPStatusCode(content, jsonEncodePos) {
...
function extractPHPArrayKeys(arrayContent) {
    const keys = [];
    let depth = 0;
    let inString = null;
    ...
    if (depth === 0)
        topLevelRanges.push([rangeStart, arrayContent.length]);
...
export function extractPHPResponseShapes(content) {
    const successKeys = [];
    const errKeys = [];
