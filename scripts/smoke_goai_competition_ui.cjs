  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const state = await window.webContents.executeJavaScript(`
      (async () => {
        ...
        return { activeMenu: openxnetApp.activeMenu, enterpriseTab: openxnetApp.enterpriseTab };
      })()
    `, true);
    stablePasses = state.activeMenu === activeMenu && state.enterpriseTab === enterpriseTab
      ? stablePasses + 1
      : 0;
...
/** 渲染事件指挥完整闭环；输入窗口和 Runtime 快照，无返回。 */
async function renderCompetitionWorkbench(window, snapshot) {
  await window.webContents.executeJavaScript(`
    (async () => {
      openxnetApp.activeMenu = 'enterprise';
...
/** 返回沙盘视觉测试数据；无输入，包含一个项目楼层、三名员工和操作审计消息。 */
  ...
  const messages = [
    {
      ...
      taskId: null,
      traceId: "trace-goai-feature-drift",
      operation: null,
    ...
    },
    {
      ...
      mentions: [],
      taskId: "action-goai-feature-drift",
      traceId: "trace-goai-feature-drift",
      operation: {
        ...
        approvalId: "approval-goai-feature-drift",
        invocationIds: ["invocation-dataops", "invocation-mlops"],
        evidenceIds: ["evidence-lineage", "evidence-probe"],
...
/** 渲染项目楼层和企业群聊；输入窗口与沙盘数据，无返回。 */
async function renderEnterpriseSandbox(window, fixture) {
  await window.webContents.executeJavaScript(`
    (async () => {
      openxnetApp.activeMenu = 'enterprise';
...
async function runSmoke() {
  ...
  try {
    ...
    const sandboxFixture = createSandboxFixture();
    await renderEnterpriseSandbox(window, sandboxFixture);
    ...
    const sandboxContract = await window.webContents.executeJavaScript(`
      (() => ({
        ...
        elements: ['.enterprise-sandbox-navigator', '.enterprise-sandbox-stage', '.enterprise-sandbox-panel--chat', '#enterprise-3d-container canvas'].map((selector) => {
          const element = document.querySelector(selector);
