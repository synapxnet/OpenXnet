# -*- coding: utf-8 -*-
"""构建自包含的 OpenXnet Live Worker 功能包目录。"""
...
def run_pyinstaller() -> None:
    """使用当前 Python 环境构建 Live Worker；无输入和返回值，构建失败时抛出 CalledProcessError。"""

    subprocess.run(
        [sys.executable, "-m", "PyInstaller", "live-worker.spec", "--clean", "--noconfirm"],
        cwd=PROJECT_ROOT,
        check=True,
    )
...
def list_payload_files(root: Path) -> list[Path]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            raise ValueError(f"Live feature pack cannot contain a symbolic link: {path}")
        if path.is_file():
            if path.name != "manifest.json" or path.parent != root:
                files.append(path)
        elif not path.is_dir():
            raise ValueError(f"Unsupported Live feature-pack entry: {path}")
    return files
...
def create_file_records(root: Path) -> list[dict[str, Any]]:
    """生成载荷完整性记录；输入根目录，返回路径、大小和哈希列表，不修改文件。"""
...
def resolve_worker_executable(distribution_root: Path) -> Path:
    ...
    executable_name = "live-worker.exe" if platform.system() == "Windows" else "live-worker"
    executable = distribution_root / executable_name
    if not executable.is_file():
        raise FileNotFoundError(f"Live Worker executable was not built: {executable}")
    return executable


def build_feature_pack(output_override: Path | None, skip_pyinstaller: bool) -> dict[str, Any]:
    """构建并哈希 Live Pack；输入输出覆盖和构建开关，返回摘要，任何完整性失败都会抛出异常。"""

    if not skip_pyinstaller:
        run_pyinstaller()
    desktop_package = read_json(PROJECT_ROOT / "package.json")
    pack_version = f"{LIVE_WORKER_VERSION}-openxnet.{desktop_package['version']}"
    ...
    distribution_root = PROJECT_ROOT / "dist" / "live-worker"
