# -*- coding: utf-8 -*-
"""Build a self-contained OpenXnet Connector Worker feature-pack directory."""
...
CONNECTOR_WORKER_VERSION = "1.6.0"
...
def run_pyinstaller() -> None:
    """Build the one-folder Connector Worker with the current Python environment."""
...
def list_payload_files(root: Path) -> list[Path]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            raise ValueError(f"Connector feature pack cannot contain a symbolic link: {path}")
        if path.is_file():
            if path.name != "manifest.json" or path.parent != root:
                files.append(path)
        elif not path.is_dir():
            raise ValueError(f"Unsupported Connector feature-pack entry: {path}")
    return files
...
def resolve_worker_executable(distribution_root: Path) -> Path:
    ...
    executable_name = "connector-worker.exe" if platform.system() == "Windows" else "connector-worker"
    executable = distribution_root / executable_name
    if not executable.is_file():
        raise FileNotFoundError(f"Connector Worker executable was not built: {executable}")
    return executable


def build_feature_pack(output_override: Path | None, skip_pyinstaller: bool) -> dict[str, Any]:
    """Build, copy, hash, and describe one platform Connector Worker pack."""

    if not skip_pyinstaller:
        run_pyinstaller()
    desktop_package = read_json(PROJECT_ROOT / "package.json")
    pack_version = f"{CONNECTOR_WORKER_VERSION}-openxnet.{desktop_package['version']}"
    ...
    distribution_root = PROJECT_ROOT / "dist" / "connector-worker"
