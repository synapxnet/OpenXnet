...
# SPDX-FileCopyrightText: Rapptz/discord.py
...
# ==============================================================================
"""
Discord Bot 管理 — 基于 discord.py 的多频道消息路由与管理。

原始项目: Rapptz/discord.py (https://github.com/Rapptz/discord.py)
License: MIT
...
from py.get_setting import convert_to_opus_simple, get_port
...
class DiscordBotManager:
    ...
    def _run_bot_thread(self, config: DiscordBotConfig):
        ...
        try:
            ...
            async def main_startup():
                try:
                    behavior_data, target_ids = await resolve_connector_behavior_snapshot(
                        config.behaviorSettings,
                        config.behaviorTargetChatIds,
                        "discordBotConfig",
                    )
                    config.toolMemorandumEnabled = await resolve_connector_tool_memorandum_enabled(
                        config.toolMemorandumEnabled,
                    )
    ...
    def update_behavior_config(self, config: DiscordBotConfig):
        ...
        if self.bot_client:
            ...
            self.bot_client.config.toolMemorandumEnabled = config.toolMemorandumEnabled
...
class DiscordClient(discord.Client):
    ...
    async def _handle_message(self, msg: discord.Message):
        ...
        async for chunk in stream:
            ...
            if tool_link and self.config.toolMemorandumEnabled:
    ...
    async def _transcribe_audio(self, audio_bytes: bytes, filename: str) -> Optional[str]:
        """通过 Connector Voice 边界转写 Discord 音频附件。"""

        try:
            return await transcribe_connector_audio(audio_bytes, filename, "auto")
        except ConnectorVoiceClientError as error:
            logging.warning("Discord ASR 请求失败，错误码: %s", error.code)
            return None
    ...
    async def _send_voice(self, msg: discord.Message, text: str):
        ...
        try:
            opus = await synthesize_connector_speech(payload)
        except ConnectorVoiceClientError as error:
            logging.warning("Discord TTS 请求失败，错误码: %s", error.code)
