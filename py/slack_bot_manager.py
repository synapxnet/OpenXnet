...
# SPDX-FileCopyrightText: slackapi/bolt-python
...
# ==============================================================================
"""
Slack Bot 管理 — 基于 slack-bolt 的 Slack 消息路由与管理。

原始项目: slackapi/bolt-python (https://github.com/slackapi/bolt-python)
License: MIT
...
from py.connector_voice_client import ConnectorVoiceClientError, synthesize_connector_speech
...
class SlackBotManager:
    ...
    async def _handle_message(self, event: dict, web_client: AsyncWebClient):
        ...
        client_ai = create_connector_chat_client()
    ...
    async def _send_voice(self, cid: str, text: str, web_client: AsyncWebClient):
        ...
        try:
            ...
            audio = await synthesize_connector_speech(payload)
            ...
            logging.error("Slack TTS 请求失败，错误码: %s", error.code)
