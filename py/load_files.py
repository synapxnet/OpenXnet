...
# ==============================================================================
"""
文件加载器 — 支持 PDF/DOCX/TXT 等多格式文档解析。
...
from py.get_setting import BLOCKLIST, TOOL_TEMP_DIR, UPLOAD_FILES_DIR
import zipfile
# 平台检测
...
async def handle_url(url):
    ...
    if 'uploaded_files' in parsed_url.path or 'tool_temp' in parsed_url.path:
        file_path = resolve_internal_file_url(url)
        try:
            content = await asyncio.to_thread(file_path.read_bytes)
            return content, ext
        except Exception as error:
            raise RuntimeError("内部文件读取失败") from error
...
def resolve_internal_file_url(url: str) -> Path:
    """解析应用内部文件 URL；输入 URL，返回允许目录内普通文件，路径越界或符号链接时抛出 ValueError。"""

    parsed = urlparse(str(url or ""))
    segments = [unquote(segment) for segment in parsed.path.split("/") if segment]
    root: Path | None = None
    marker_index = -1
    for marker, directory in (("uploaded_files", UPLOAD_FILES_DIR), ("tool_temp", TOOL_TEMP_DIR)):
        if marker in segments:
            root = Path(directory).resolve()
            marker_index = segments.index(marker)
            break
    relative_segments = segments[marker_index + 1:] if marker_index >= 0 else []
    if root is None or len(relative_segments) != 1:
        raise ValueError("内部文件 URL 无效")
    filename = relative_segments[0]
    ...
    try:
        ...
        raise ValueError("内部文件 URL 无效") from error
    if not candidate.is_file():
        raise FileNotFoundError("内部文件不存在")
    return candidate
...
async def handle_office_document(content, ext):
    """通过 Document Worker 提取办公文档并保留旧异步入口。"""

    if IS_MAC and ext in ['pages', 'numbers', 'key']:
        raise NotImplementedError(f"iWork格式暂不支持自动解析，请手动导出为通用格式")
    from py.document_worker_client import extract_office_document

    return await extract_office_document(content, ext)
...
def _process_doc(content):
    """通过 Windows Word COM API 同步提取旧 DOC 文本。"""
