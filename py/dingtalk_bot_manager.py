...
# SPDX-FileCopyrightText: open-dingtalk
...
# ==============================================================================
"""
钉钉 Bot 管理 — 基于钉钉开放平台 SDK 的消息处理。

原始项目: open-dingtalk (https://open.dingtalk.com)
License: Apache-2.0
...
class DingtalkClientLogic:
    ...
    async def on_message(self, raw_data: dict, incoming_message: ChatbotMessage, handler: DingtalkInternalHandler):
        ...
        # --- D. AI 调用与流式输出 ---
        ai_client = create_connector_chat_client()
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload) as resp:
                    if resp.status == 200:
                        ...
                        logging.warning("Dingtalk access-token request failed with status %s.", resp.status)
        except Exception as e:
            logging.warning("Dingtalk access-token request failed: %s.", type(e).__name__)
        return None
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, headers=headers) as resp:
                    ...
                    if resp.status == 200 and result.get("processQueryKey"):
                        ...
                        logging.warning("Dingtalk delivery failed with status %s.", resp.status)
                        
        except Exception as e:
            logging.error("Dingtalk delivery failed: %s.", type(e).__name__)
