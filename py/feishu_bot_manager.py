...
# SPDX-FileCopyrightText: larksuite/oapi-sdk-python
...
# ==============================================================================
"""
飞书 Bot 管理 — 基于 lark-oapi 的飞书机器人消息路由与事件处理。

原始项目: larksuite/oapi-sdk-python (https://github.com/larksuite/oapi-sdk-python)
License: MIT
...
class FeishuBotManager:
    ...
    def _run_bot_thread(self, config):
        ...
        try:
            ...
            try:
                behavior_data, target_ids = asyncio.run(
                    resolve_connector_behavior_snapshot(
                ...
                config.toolMemorandumEnabled = asyncio.run(
                    resolve_connector_tool_memorandum_enabled(
...
class FeishuClient:
    ...
    async def _transcribe_audio(self, audio_data: bytes, file_key: str) -> Optional[str]:
        """通过 Connector Voice 边界转写飞书音频并返回非空文本。"""

        try:
            text = await transcribe_connector_audio(
                audio_data,
                f"{file_key}.ogg",
                "auto",
            )
            if text:
                logging.info("飞书 ASR 识别成功。")
            else:
                logging.warning("飞书 ASR 识别结果为空。")
            return text
        except ConnectorVoiceClientError as error:
            logging.warning("飞书 ASR 请求失败，错误码: %s", error.code)
            return None
    ...
    async def _send_voice(self, original_msg, text):
        """通过 Connector Voice 边界生成 Opus 并发送飞书语音消息。"""
        ...
        try:
            ...
            logging.warning("飞书 TTS 请求失败，错误码: %s", error.code)
            await self._send_text(original_msg, "语音功能暂时不可用，请稍后重试")
        except Exception as error:
            logging.warning("发送飞书语音异常: %s", type(error).__name__)
            await self._send_text(original_msg, "语音功能暂时不可用，请稍后重试")
