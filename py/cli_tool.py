...
from py.get_setting import SKILLS_DIR, load_settings
...
async def edit_file_tool_local(path: str, content: str) -> str:
    ...
    try:
        ...
        try:
            from py.git_shadow import git_checkpoint
            await git_checkpoint(cwd, path, "edit_file", f"Before full write to {path}")
        except Exception:
            pass  # 静默失败，不阻断编辑
...
async def edit_file_patch_tool_local(path: str, old_string: str, new_string: str) -> str:
    ...
    try:
        ...
        try:
            from py.git_shadow import git_checkpoint
            await git_checkpoint(cwd, path, "edit_file_patch", f"Before patch on {path}")
        except Exception:
            pass  # 静默失败，不阻断编辑

        # [v0.5.2 P0] 创建 .bak 备份（之前此工具完全没有备份）
        try:
            import shutil
            backup_path = target.with_suffix(target.suffix + ".bak")
            shutil.copy2(target, backup_path)
...
EXTERNAL_CLI_NATIVE_CREDENTIAL_ENV_NAMES = frozenset({
    "ANTHROPIC_API_KEY",
...
MAX_EXTERNAL_CLI_SECRET_LENGTH = 65_536


def _is_external_cli_private_environment_name(name: object) -> bool:
    """Return whether one environment name belongs to an application secret envelope."""

    normalized_name = str(name or "").strip().upper()
    return normalized_name.startswith("OPENXNET_") or normalized_name.endswith(
        "_CREDENTIALS_B64"
    )


def _build_external_cli_environment(
    extra: dict[str, str] | None = None,
    *,
    preserve_native_credentials: bool,
) -> dict[str, str]:
    """Build one child environment without OpenXnet private runtime credentials."""

    environment: dict[str, str] = {}
    for raw_name, raw_value in os.environ.items():
        name = str(raw_name)
        normalized_name = name.upper()
        if _is_external_cli_private_environment_name(normalized_name):
            continue
        if (
            not preserve_native_credentials
            and normalized_name in EXTERNAL_CLI_NATIVE_CREDENTIAL_ENV_NAMES
        ):
            continue
        environment[name] = str(raw_value)
    for raw_name, raw_value in (extra or {}).items():
        name = str(raw_name).strip()
        if name and not _is_external_cli_private_environment_name(name):
            environment[name] = str(raw_value)
    return environment


def _external_cli_secret_values(environment: dict[str, str]) -> tuple[str, ...]:
    """Return bounded credential values that must be redacted from CLI output."""

    secrets = {
        str(value)
        for name, value in environment.items()
        if name.upper() in EXTERNAL_CLI_NATIVE_CREDENTIAL_ENV_NAMES
        and 0 < len(str(value)) <= MAX_EXTERNAL_CLI_SECRET_LENGTH
    }
    return tuple(sorted(secrets, key=len, reverse=True))


def _redact_external_cli_text(value: object, secrets: tuple[str, ...]) -> str:
    """Remove exact selected credential values from one external CLI message."""
...
def _is_external_cli_loopback(hostname: str) -> bool:
    """Return whether one external CLI endpoint host is explicit loopback."""
...
def _normalize_external_cli_base_url(value: object) -> str:
    """Validate one external CLI base URL with HTTPS required off loopback."""

    normalized = str(value or "").strip().rstrip("/")
    if not normalized or len(normalized) > 2_048:
        raise RuntimeError("External CLI provider endpoint is invalid.")
    try:
        parsed = urlsplit(normalized)
        port = parsed.port
    except ValueError as error:
        raise RuntimeError("External CLI provider endpoint is invalid.") from error
    if (
        ...
        or parsed.query
        or parsed.fragment
        or (parsed.scheme == "http" and not _is_external_cli_loopback(parsed.hostname))
...
def _build_external_cli_custom_environment(
    configuration: dict,
    *,
    base_url_name: str,
    api_key_name: str,
    ...
    """Build one exact custom-provider environment or a fixed validation error."""

    if not configuration.get("enabled"):
        return _build_external_cli_environment(
    ...
    if (
        ...
        or len(model) > 512
        or not api_key
        or len(api_key) > MAX_EXTERNAL_CLI_SECRET_LENGTH
        or any(ord(character) < 32 or ord(character) == 127 for character in api_key)
    ):
        return {}, "External CLI provider configuration is invalid."
    try:
        base_url = _normalize_external_cli_base_url(configuration.get("base_url"))
    except RuntimeError:
        return {}, "External CLI provider configuration is invalid."
    return _build_external_cli_environment({
        ...
        model_name: model,
    }, preserve_native_credentials=False), None

MAX_CLAUDE_CLI_STREAM_LINE_BYTES = 1024 * 1024
MAX_CLAUDE_CLI_PROMPT_LENGTH = 2 * 1024 * 1024
...
def _build_claude_cli_args(permission_mode: object) -> list[str]:
    """构建 Claude CLI 固定参数；输入权限模式，返回无提示词和凭据的参数数组，非法模式降级为 default。"""

    normalized_mode = str(permission_mode or "default").strip() or "default"
    if normalized_mode == "cowork":
        normalized_mode = "bypassPermissions"
    if normalized_mode not in CLAUDE_CLI_PERMISSION_MODES:
...
def _extract_claude_cli_texts(value: object) -> list[str]:
    """提取 Claude stream-json 文本；输入未知事件，返回有界文本块数组，非公开事件返回空数组。"""
    ...
    if value.get("type") == "assistant":
        ...
        for block in content[:128]:
            ...
            if text:
                texts.append(text[:MAX_CLAUDE_CLI_STREAM_LINE_BYTES])
        return texts
    if value.get("type") == "result" and isinstance(value.get("result"), str):
        result = str(value["result"])
        return [result[:MAX_CLAUDE_CLI_STREAM_LINE_BYTES]] if result else []
    return []


async def _discard_external_cli_stream(stream: asyncio.StreamReader | None) -> None:
    """丢弃外部 CLI 诊断流；输入可空流，无返回，不保留可能包含本机路径或供应商诊断的内容。"""
...
async def _stream_claude_cli(
    ...
    process = await asyncio.create_subprocess_exec(
        executable,
        *_build_claude_cli_args(permission_mode),
        stdin=asyncio.subprocess.PIPE,
        stdout=asyncio.subprocess.PIPE,
        stderr=asyncio.subprocess.PIPE,
        cwd=cwd,
        env={**environment, "CLAUDE_CODE_ENTRYPOINT": "openxnet"},
        limit=MAX_CLAUDE_CLI_STREAM_LINE_BYTES,
    )
    ...
    try:
        ...
        while True:
            ...
            if not raw_line:
                break
            if len(raw_line) > MAX_CLAUDE_CLI_STREAM_LINE_BYTES:
                raise RuntimeError("Claude Code output exceeds its line budget.")
            try:
            ...
            if isinstance(event, dict) and event.get("type") == "result" and emitted_assistant_text:
                continue
            for text in _extract_claude_cli_texts(event):
...
async def claude_code(prompt) -> str | AsyncIterator[str]:
    """运行已安装的 Claude Code CLI；输入提示词，返回脱敏文本流，配置或启动失败时返回固定错误。"""
    ...
    runtime_environment, configuration_error = _build_external_cli_custom_environment(
        ccSettings,
    ...
    secrets = _external_cli_secret_values(runtime_environment)
    executable = shutil.which("claude") or "claude"

    async def _stream():
        """封装 Claude CLI 输出；无输入，返回异步文本流，异常转换为固定公开错误。"""

        try:
            async for text in _stream_claude_cli(
                executable,
                str(prompt or ""),
                str(cwd),
                runtime_environment,
                ccSettings.get("permissionMode", "default"),
                secrets,
            ):
                yield text
        except Exception:
...
def _normalize_codex_permission_mode(permission_mode: str) -> str:
...
def _build_codex_exec_args(permission_mode: str, last_message_path: str) -> list[str]:
    ...
    elif normalized in {"bypassPermissions", "cowork"}:
        args.append("--dangerously-bypass-approvals-and-sandbox")
    else:
        args.extend(["-c", "approval_policy=never", "-c", "sandbox_mode=read-only"])
    return args


def _resolve_codex_native_executable() -> str | None:
    candidates = []
    explicit = str(os.environ.get("OPENXNET_CODEX_EXECUTABLE", "")).strip()
    if explicit:
        candidates.append(explicit)
    command_names = ("codex.exe", "codex") if platform.system() == "Windows" else ("codex", "codex.exe")
    ...
    if platform.system() == "Windows":
        ...
        pattern = os.path.join(program_files, "WindowsApps", "OpenAI.Codex_*", "app", "resources", "codex.exe")
        candidates.extend(sorted(std_glob.glob(pattern), reverse=True))
...
def _wsl_is_ready() -> bool:
    ...
    try:
        result = subprocess.run(
            [wsl_exe, "bash", "-lc", "printf '__OPENXNET_WSL_READY__'"],
        ...
        return False
    return result.returncode == 0 and b"__OPENXNET_WSL_READY__" in (result.stdout or b"")
...
def _prepare_codex_runtime_home(codex_home: Path, oc_settings: dict) -> tuple[dict, str | None]:
    """Write secret-free custom Codex metadata and return its selected key."""

    if not oc_settings.get("enabled"):
        return {}, None
    runtime_environment, configuration_error = _build_external_cli_custom_environment(
        oc_settings,
        base_url_name="OPENAI_BASE_URL",
    ...
    if configuration_error:
        return {}, configuration_error
    base_url = runtime_environment["OPENAI_BASE_URL"]
    model = runtime_environment["OPENAI_MODEL"]
    api_key = runtime_environment["OPENAI_API_KEY"]
    ...
    return {"OPENAI_API_KEY": api_key}, None


def _read_codex_last_message(last_message_path: Path) -> str:
    try:
...
def _build_codex_runtime_hint() -> str:
    if platform.system() == "Windows":
        return (
            "OpenAI Codex CLI is not runnable in the current environment. "
            "Please make sure `codex` is installed and runnable, or install it in WSL2 with "
...
async def _stream_codex_native(
    ...
    final_message = _read_codex_last_message(last_message_path)
    if final_message and not output_received:
        yield _redact_external_cli_text(final_message, secrets)


def _build_codex_wsl_environment(environment: dict[str, str]) -> dict[str, str]:
    """Configure WSL environment forwarding without embedding values in argv."""

    result = dict(environment)
    forwarded = []
    existing = str(result.get("WSLENV") or "").split(":")
    forwarded.extend(entry for entry in existing if entry)
    if result.get("OPENAI_API_KEY"):
        forwarded.append("OPENAI_API_KEY")
    if result.get("CODEX_HOME"):
        forwarded.append("CODEX_HOME/p")
    deduplicated = []
    seen = set()
    ...
    result["WSLENV"] = ":".join(deduplicated)
    return result


async def _stream_codex_wsl(
    wsl_exe: str,
    permission_mode: str,
    cwd: str,
    env: dict,
    ...
    script = "\n".join([
        "set -e",
        f"cd {shlex.quote(_to_wsl_path(cwd))}",
        f"cat {shlex.quote(_to_wsl_path(str(prompt_file)))} | {wsl_command}",
    ])
    wsl_environment = _build_codex_wsl_environment(env)
    process = await asyncio.create_subprocess_exec(
        wsl_exe,
        "bash",
        "-lc",
...
async def openai_codex(prompt: str) -> str | AsyncIterator[str]:
    """Run Codex with CLI-owned auth or one environment-only Provider key."""

    settings = await load_settings()
    cwd = settings.get("CLISettings", {}).get("cc_path")
    oc_settings = settings.get("ocSettings", {})
    if not cwd:
        return "No working directory."

    permission_mode = oc_settings.get("permissionMode", "default")
    temp_root = Path(tempfile.mkdtemp(prefix="openxnet-codex-"))
    codex_home = temp_root / ".codex"
    prompt_file = temp_root / "prompt.txt"
    last_message_path = temp_root / "last_message.txt"

    extra_env, config_error = _prepare_codex_runtime_home(codex_home, oc_settings)
    if config_error:
        shutil.rmtree(temp_root, ignore_errors=True)
        return config_error
    custom_provider_enabled = bool(oc_settings.get("enabled"))
    runtime_env = _build_external_cli_environment(
        extra_env,
        preserve_native_credentials=not custom_provider_enabled,
    )
    if custom_provider_enabled:
        runtime_env["CODEX_HOME"] = str(codex_home)
    secrets = _external_cli_secret_values(runtime_env)

    prompt_file.write_text(prompt or "", encoding="utf-8", newline="\n")
    args = _build_codex_exec_args(permission_mode, str(last_message_path))

    async def _stream():
        try:
            if platform.system() == "Windows" and _wsl_is_ready():
                wsl_exe = shutil.which("wsl.exe") or shutil.which("wsl")
                if not wsl_exe:
                    yield _build_codex_runtime_hint()
                    return
                async for out in _stream_codex_wsl(
                    wsl_exe,
                    permission_mode,
                    cwd,
                    runtime_env,
            ...
            if executable:
                try:
                    async for out in _stream_codex_native(
                        executable,
                        args,
                        cwd,
                        runtime_env,
                    ...
                    if platform.system() == "Windows":
                        wsl_exe = shutil.which("wsl.exe") or shutil.which("wsl")
                        if wsl_exe and _wsl_is_ready():
                            yield "[INFO] Native Codex launch failed; attempting WSL fallback."
                            async for out in _stream_codex_wsl(
                                wsl_exe,
                                permission_mode,
                                cwd,
                                ...
                                yield out
                            return
                    yield "[ERROR] Failed to launch Codex executable."
                    yield _build_codex_runtime_hint()
                    return

            yield _build_codex_runtime_hint()
        except Exception:
            yield "OpenAI Codex request failed."
        finally:
            shutil.rmtree(temp_root, ignore_errors=True)
...
openai_codex_tool = {
    ...
    "function": {
        "name": "openai_codex",
        "description": f"Interact with OpenAI Codex Agent. {cli_info}",
        "parameters": {"type": "object", "properties": {"prompt": {"type": "string"}}, "required": ["prompt"]}
    }
}
