...
# SPDX-FileCopyrightText: tencent-connect/botpy
...
# ==============================================================================
"""
QQ Bot 管理 — 基于官方 botpy SDK 的 QQ 机器人消息处理。

原始项目: tencent-connect/botpy (https://github.com/tencent-connect/botpy)
License: MIT
...
from py.connector_settings import resolve_connector_tool_memorandum_enabled
import time
...
class QQBotManager:
    ...
    def _run_bot_thread(self, config):
        ...
        try:
            ...
            config.toolMemorandumEnabled = self.loop.run_until_complete(
    ...
    def update_behavior_config(self, config: QQBotConfig):
        """热更新 QQ 的无密钥运行配置；输入完整配置，无返回，不重启机器人或修改会话记录。"""
