...
import { APPLICATION_VOICE_CREDENTIAL_CHANNELS } from "../contracts/application-voice-credentials";
import type { IpcMainLike } from "./register-core-ipc";
import { registerApplicationVoiceCredentialsIpc } from "./register-application-voice-credentials-ipc";

type Handler = (event: unknown, ...arguments_: readonly unknown[]) => unknown;

/** Minimal IPC harness used by voice credential adapter tests. */
...
test("registerApplicationVoiceCredentialsIpc authorizes and cleans up every handler", () => {
  const ipcMain = new TestIpcMain();
  const authorizedEvent = { authorized: true };
  const calls: string[] = [];
  const cleanup = registerApplicationVoiceCredentialsIpc({
    ipcMain,
    voiceCredentials: {
      getSnapshot: () => {
        calls.push("get");
  ...
  assert.deepEqual(
    ipcMain.invoke(APPLICATION_VOICE_CREDENTIAL_CHANNELS.getSnapshot, authorizedEvent),
    { kind: "snapshot" },
  );
  assert.deepEqual(
    ipcMain.invoke(APPLICATION_VOICE_CREDENTIAL_CHANNELS.save, authorizedEvent, {}),
    { kind: "saved" },
  );
  assert.deepEqual(calls, ["get", "save"]);
  assert.throws(
    () => ipcMain.invoke(APPLICATION_VOICE_CREDENTIAL_CHANNELS.getSnapshot, {}),
    /not authorized/,
  );
  cleanup();
