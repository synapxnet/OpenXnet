...
import { APPLICATION_IMAGE_HOST_CREDENTIAL_CHANNELS } from "../contracts/application-image-host-credentials";
import { registerApplicationImageHostCredentialsIpc } from "./register-application-image-host-credentials-ipc";
import type { IpcMainLike } from "./register-core-ipc";

type Handler = (event: unknown, ...arguments_: readonly unknown[]) => unknown;

/** Minimal IPC harness used by image-host credential adapter tests. */
...
test("Image-host credential IPC authorizes every handler and cleans up", () => {
  const ipcMain = new TestIpcMain();
  const authorizedEvent = { authorized: true };
  const cleanup = registerApplicationImageHostCredentialsIpc({
    ipcMain,
    imageHostCredentials: {
      getSnapshot: () => ({ kind: "image-host" }) as never,
      save: () => ({ kind: "image-host-saved" }) as never,
    },
  ...
  assert.deepEqual(
    ipcMain.invoke(APPLICATION_IMAGE_HOST_CREDENTIAL_CHANNELS.getSnapshot, authorizedEvent),
    { kind: "image-host" },
  );
  assert.deepEqual(
    ipcMain.invoke(APPLICATION_IMAGE_HOST_CREDENTIAL_CHANNELS.save, authorizedEvent, {}),
    { kind: "image-host-saved" },
  );
  assert.throws(
    () => ipcMain.invoke(APPLICATION_IMAGE_HOST_CREDENTIAL_CHANNELS.getSnapshot, {}),
    /not authorized/,
  );
  cleanup();
