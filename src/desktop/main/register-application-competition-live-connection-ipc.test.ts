 ...
 * Live 接入 IPC 授权回归测试 / Live connection IPC authorization regression tests.
...
import { APPLICATION_COMPETITION_LIVE_CONNECTION_CHANNELS, type ApplicationCompetitionLiveConnectionSnapshot } from "../contracts/application-competition-live-connection";
...
import { registerApplicationCompetitionLiveConnectionIpc } from "./register-application-competition-live-connection-ipc";

test("every connection IPC channel authorizes before accessing the service and cleans up", /** 验证本项 Live 接入行为与边界。 / Verify this Live access behavior and boundary. */ async () => {
  ...
  const calls: string[] = [];
  const snapshot: ApplicationCompetitionLiveConnectionSnapshot = {
    enabled: false, endpoint: "", workspaceId: "", credentialConfigured: false, storageAvailable: true, source: "none", access: null,
  };
  const cleanup = registerApplicationCompetitionLiveConnectionIpc({
    ipcMain,
    ...
    service: {
      ...
      getSnapshot(): ApplicationCompetitionLiveConnectionSnapshot { calls.push("get"); return snapshot; },
  ...
  for (const channel of Object.values(APPLICATION_COMPETITION_LIVE_CONNECTION_CHANNELS)) {
...
test("typed and installed preload expose the same restricted connection operations", /** 验证真实两份预加载桥接，防止安装包缺少配置入口。 / Verify both actual preload bridges so the installed app cannot lose connection operations. */ async () => {
  for (const packaged of [false, true]) {
    ...
    const request = { endpoint: "https://demo.example/adapter/", workspaceId: "workspace-test", accessCode: `oxlive_${"a".repeat(64)}` };
    const savedRequest = { ...request, enabled: true };
    await api.getApplicationCompetitionLiveConnection();
    await api.testApplicationCompetitionLiveConnection(request);
    await api.saveApplicationCompetitionLiveConnection(savedRequest);
    await api.clearApplicationCompetitionLiveConnection();
    assert.deepEqual(calls, [
      [APPLICATION_COMPETITION_LIVE_CONNECTION_CHANNELS.get],
      [APPLICATION_COMPETITION_LIVE_CONNECTION_CHANNELS.test, request],
      [APPLICATION_COMPETITION_LIVE_CONNECTION_CHANNELS.save, savedRequest],
      [APPLICATION_COMPETITION_LIVE_CONNECTION_CHANNELS.clear],
    ]);
  }
});
