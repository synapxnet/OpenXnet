...
import { APPLICATION_DELIVERY_CREDENTIAL_CHANNELS } from "../contracts/application-delivery-credentials";
import { registerApplicationDeliveryCredentialsIpc } from "./register-application-delivery-credentials-ipc";
import type { IpcMainLike } from "./register-core-ipc";

type Handler = (event: unknown, ...arguments_: readonly unknown[]) => unknown;

/** Minimal IPC harness used by delivery credential adapter tests. */
...
test("delivery credential IPC authorizes every handler and cleans up", () => {
  const ipcMain = new TestIpcMain();
  const authorizedEvent = { authorized: true };
  const cleanup = registerApplicationDeliveryCredentialsIpc({
    ipcMain,
    deliveryCredentials: {
      getSnapshot: () => ({ kind: "delivery" }) as never,
      save: () => ({ kind: "delivery-saved" }) as never,
    },
  ...
  assert.deepEqual(
    ipcMain.invoke(APPLICATION_DELIVERY_CREDENTIAL_CHANNELS.getSnapshot, authorizedEvent),
    { kind: "delivery" },
  );
  assert.deepEqual(
    ipcMain.invoke(APPLICATION_DELIVERY_CREDENTIAL_CHANNELS.save, authorizedEvent, {}),
    { kind: "delivery-saved" },
  );
  assert.throws(
    () => ipcMain.invoke(APPLICATION_DELIVERY_CREDENTIAL_CHANNELS.save, {}, {}),
    /not authorized/,
  );
  cleanup();
