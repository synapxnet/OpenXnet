 ...
 * Live 接入与系统加密存储 / Live connection and OS-encrypted storage.
...
import type {
  ApplicationCompetitionLiveConnectionAccess,
  ApplicationCompetitionLiveConnectionCode,
  ApplicationCompetitionLiveConnectionResult,
  ApplicationCompetitionLiveConnectionSnapshot,
...
import { COMPETITION_TOOL_REGISTRY } from "./competition-tool-registry";
import { APPLICATION_COMPETITION_SCENARIO_TYPES } from "../contracts/application-competition-runtime";

const MAX_BYTES = 64 * 1024;
const TIMEOUT_MS = 8_000;
const ACCESS_CODE_PATTERN = /^oxlive_[a-f0-9]{64}$/u;
...
export interface ApplicationCompetitionLiveSavedConnection {
...
interface StoredConnection extends ApplicationCompetitionLiveSavedConnection {
  readonly schema: "openxnet.desktop.agentteams-live-connection.v1";
  readonly ownerId: string;
}

/** 服务依赖；账号身份必须来自已认证 Main。 / Dependencies resolve account ownership from authenticated Main state. */
export interface ApplicationCompetitionLiveConnectionServiceOptions {
...
class ConnectionError extends Error {
  ...
  public constructor(public readonly code: ApplicationCompetitionLiveConnectionCode) { super(code); }
}
...
function parseAccess(value: unknown): ApplicationCompetitionLiveConnectionAccess {
  if (!isRecord(value) || value.schema !== "openxnet.agentteams.live-access.v1"
      ...
      || typeof value.workspaceId !== "string" || !WORKSPACE_PATTERN.test(value.workspaceId)
      ...
      || !Array.isArray(value.modes) || value.modes.length !== 1 || value.modes[0] !== "live"
      || value.requiredTeamRuntime !== "agentteams"
      || typeof value.serviceReady !== "boolean" || typeof value.approvalReady !== "boolean"
      || !Array.isArray(value.platforms) || value.platforms.length !== 3
      || !Array.isArray(value.checks) || value.checks.length > 32) {
    throw new ConnectionError("INVALID_RESPONSE");
  }
  const allowedTools = parseScope(value.allowedTools, COMPETITION_TOOL_REGISTRY.map(/** 提取固定工具名。 / Extract a fixed tool name. */ (entry) => entry.name));
  const allowedScenarios = parseScope(value.allowedScenarios, APPLICATION_COMPETITION_SCENARIO_TYPES);
  const platforms = value.platforms.map(
    /** 仅保留真实平台的三个独立就绪信号。 / Retain the three independent readiness signals for each platform. */
    (entry): ApplicationCompetitionLiveConnectionAccess["platforms"][number] => {
      if (!isRecord(entry) || !["aiops", "dataops", "mlops"].includes(String(entry.platform))
          || typeof entry.configured !== "boolean" || typeof entry.reachable !== "boolean" || typeof entry.identityMatched !== "boolean"
          || (entry.identityMatched && !entry.reachable) || (entry.reachable && !entry.configured)) throw new ConnectionError("INVALID_RESPONSE");
      return Object.freeze({ platform: entry.platform as "aiops" | "dataops" | "mlops", configured: entry.configured, reachable: entry.reachable, identityMatched: entry.identityMatched });
    });
  if (new Set(platforms.map(/** 提取身份以检查重复。 / Extract identity for uniqueness checks. */ (entry) => entry.platform)).size !== 3) throw new ConnectionError("INVALID_RESPONSE");
  const checks = value.checks.map(
    /** 过滤检测结果，禁止任意异常或凭据字段进入 Renderer。 / Filter checks so arbitrary exceptions or credential fields cannot reach Renderer. */
    (entry): ApplicationCompetitionLiveConnectionAccess["checks"][number] => {
      if (!isRecord(entry) || typeof entry.id !== "string" || !/^[a-zA-Z0-9_.-]{1,100}$/u.test(entry.id)
          || typeof entry.label !== "string" || entry.label.length > 160 || /[\u0000-\u001f]/u.test(entry.label)
          || typeof entry.ready !== "boolean" || typeof entry.code !== "string" || !/^[A-Z0-9_]{1,80}$/u.test(entry.code)) throw new ConnectionError("INVALID_RESPONSE");
      return Object.freeze({ id: entry.id, label: entry.label, ready: entry.ready, code: entry.code });
    });
  if (new Set(checks.map(/** 提取检查编号。 / Extract a check identifier. */ (entry) => entry.id)).size !== checks.length
      || (value.serviceReady && (!value.approvalReady || !platforms.every(/** 汇总独立平台信号。 / Aggregate independent platform signals. */ (entry) => entry.configured && entry.reachable && entry.identityMatched)
        || !checks.every(/** 核对分项通过。 / Check individual readiness. */ (entry) => entry.ready)))) throw new ConnectionError("INVALID_RESPONSE");
  return Object.freeze({
    schema: "openxnet.agentteams.live-access.v1", grantId: value.grantId, label: value.label,
    workspaceId: value.workspaceId, expiresAt: value.expiresAt, remainingRequests: value.remainingRequests as number,
    modes: Object.freeze(["live"] as const), serviceReady: value.serviceReady,
    requiredTeamRuntime: "agentteams",
    allowedTools, allowedScenarios, platforms: Object.freeze(platforms), approvalReady: value.approvalReady, checks: Object.freeze(checks),
  });
}

/** 拒绝通配符、未知范围和重复项，返回不可变白名单。 / Reject wildcards, unknown scopes and duplicates, returning an immutable allowlist. */
function parseScope(value: unknown, known: readonly string[]): readonly string[] {
  if (!Array.isArray(value) || value.length < 1 || value.length > known.length
      || value.some(/** 拒绝非文本或未知范围。 / Reject non-text or unknown scopes. */ (entry) => typeof entry !== "string" || !known.includes(entry))
      || new Set(value).size !== value.length) throw new ConnectionError("INVALID_RESPONSE");
  return Object.freeze([...value] as string[]);
}

/** 当前账号独享的接入服务；网络测试只读，保存使用系统加密。 / Account-bound connection service with read-only probes and OS-encrypted persistence. */
export class ApplicationCompetitionLiveConnectionService {
  ...
  public constructor(private readonly options: ApplicationCompetitionLiveConnectionServiceOptions) {
  ...
  /** 仅 Main 用于禁止账号切换后退回旧部署权限，不泄漏文件内容或持有者。 / Main uses file presence to prevent legacy-authority fallback after account switches without exposing contents or ownership. */
  public hasLocalConfiguration(): boolean {
    return existsSync(this.filePath);
  }

  /** 同步读取公开配置，损坏或不可解密时返回明确状态。 / Read public status synchronously and report corruption without throwing. */
  public getSnapshot(): ApplicationCompetitionLiveConnectionSnapshot {
    ...
    return Object.freeze({
      enabled: false, endpoint: "", workspaceId: "", credentialConfigured: false,
      storageAvailable, source: "none", access: null,
      ...(storageError ? { storageError } : {}),
    });
  }

  /** 仅供 Main 请求签发路径使用，绝不通过 IPC 调用。 / Return owner-bound credentials only to Main request dispatch, never IPC. */
  public getSavedConnection(): ApplicationCompetitionLiveSavedConnection | null {
    try {
      const stored = this.readStored();
  ...
  /** 每次真实调用前验证本机授权范围；服务器仍独立验证撤销和审批。 / Validate local scope before each live call; the server still verifies revocation and approval independently. */
  public requireRuntimeConnection(workspaceId: string, scope: { readonly toolName?: string; readonly scenario?: string } = {}): ApplicationCompetitionLiveSavedConnection {
    const saved = this.getSavedConnection();
    if (!saved || !saved.enabled) throw new ConnectionError("CONNECTION_DISABLED");
    if (workspaceId !== saved.workspaceId) throw new ConnectionError("ACCESS_SCOPE_MISMATCH");
    if (Date.parse(saved.access.expiresAt) <= this.now()) throw new ConnectionError("ACCESS_EXPIRED");
    if (saved.access.remainingRequests < 1) throw new ConnectionError("ACCESS_QUOTA_EXHAUSTED");
    if (!saved.access.serviceReady) throw new ConnectionError("SERVICE_NOT_READY");
    if ((scope.toolName && !saved.access.allowedTools.includes(scope.toolName))
        || (scope.scenario && !saved.access.allowedScenarios.includes(scope.scenario))) throw new ConnectionError("SCOPE_NOT_ALLOWED");
    return saved;
  }

  /** 只验证服务和访问范围，不绑定工作空间、不落盘。 / Validate service and scope without workspace binding or persistence. */
  public async test(request: unknown): Promise<ApplicationCompetitionLiveConnectionResult> {
    ...
    try {
      ...
      const access = await this.probe(parsed, false, ownerId);
  ...
  public async save(request: unknown): Promise<ApplicationCompetitionLiveConnectionResult> {
    if (this.inFlight) return this.failure("CONNECTION_BUSY");
    this.inFlight = true;
    try {
      const ownerId = this.requireOwner();
      ...
      this.assertOwner(ownerId);
      const access = await this.probe(parsed, true, ownerId);
      await this.assertCanChange();
      this.assertOwner(ownerId);
      this.writeStored({ schema: "openxnet.desktop.agentteams-live-connection.v1", ownerId, ...parsed, access });
      return { ok: true, snapshot: this.getSnapshot(), access };
    } catch (error) { return this.failure(this.publicCode(error)); }
    finally { this.inFlight = false; }
  }
  ...
  public async clear(): Promise<ApplicationCompetitionLiveConnectionResult> {
    if (this.inFlight) return this.failure("CONNECTION_BUSY");
    this.inFlight = true;
    try {
      const ownerId = this.requireOwner();
  ...
  private parseRequest(value: unknown, ownerId: string, saving: boolean): Omit<ApplicationCompetitionLiveSavedConnection, "access"> {
    ...
    if (!WORKSPACE_PATTERN.test(workspaceId)) throw new ConnectionError("WORKSPACE_REQUIRED");
  ...
  private async probe(request: Omit<ApplicationCompetitionLiveSavedConnection, "access">, connect: boolean, ownerId: string): Promise<ApplicationCompetitionLiveConnectionAccess> {
    const controller = new AbortController();
    const timeout = setTimeout(/** 超时取消只读探测。 / Abort the read-only probe on timeout. */ () => controller.abort(), TIMEOUT_MS);
    try {
      ...
      if (!isRecord(health.capabilities) || health.capabilities.liveAccessCodes !== true) throw new ConnectionError("UNSUPPORTED_SERVICE");
      this.assertOwner(ownerId);
      const value = await this.requestJson(new URL(`api/v1/live-access/${connect ? "connect" : "check"}`, request.endpoint), {
      ...
      if (access.workspaceId !== request.workspaceId) throw new ConnectionError("ACCESS_SCOPE_MISMATCH");
  ...
  private async requestJson(url: URL, init: RequestInit): Promise<unknown> {
    ...
    if (!response.ok) {
      ...
      const liveCodes: Readonly<Record<string, ApplicationCompetitionLiveConnectionCode>> = {
      ...
      if (typeof code === "string" && Object.hasOwn(liveCodes, code)) throw new ConnectionError(liveCodes[code]!);
  ...
  private readStored(): StoredConnection | null {
    ...
    try {
      ...
      if (!isRecord(value) || value.schema !== "openxnet.desktop.agentteams-live-connection.v1" || typeof value.ownerId !== "string") throw new Error("Invalid schema");
      ...
      return { schema: "openxnet.desktop.agentteams-live-connection.v1", ownerId, enabled: value.enabled, endpoint,
  ...
  private publicCode(error: unknown): ApplicationCompetitionLiveConnectionCode {
  ...
  private failure(code: ApplicationCompetitionLiveConnectionCode): ApplicationCompetitionLiveConnectionResult {
