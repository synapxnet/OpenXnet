...
function buildRecommendationCapacityProfile(
  ...
  return {
    ...
    skill: {
      ...
      verification: ["P99 延迟不超过 300ms。", "推理队列深度回落且 GPU 不再持续饱和。", "工作负载就绪副本达到目标值。"],
...
function buildQuantitativeIterationProfile(
  ...
  return {
      ...
      steps: [
        {
          ...
          title: "构建五年训练数据集",
          description: "回溯历史行情和交易数据，按归因结论替换失效因子。",
          phase: "REPAIR",
          kind: "WRITE",
          toolName: "dataops.training.dataset.build",
          resourceId: `dataset_quant_${scenario.targetRevision}`,
          targetRevision: scenario.targetRevision,
          expectedResourceVersion: offsetResourceVersion(scenario.expectedResourceVersion, 0),
          arguments: {
            workflowInstanceUid: scenario.workflowInstanceUid,
            datasetUid: `dataset_quant_${scenario.targetRevision}`,
            historyYears: 5,
            addedFactors: ["momentum_20d", "momentum_60d"],
            removedFactors: ["value_30d", "book_to_price", "earnings_yield"],
          },
          dependsOn: [],
        },
        {
          ...
          arguments: {
            ...
            addedFeatures: ["momentum_20d", "momentum_60d"],
            removedFeatures: ["value_30d", "book_to_price", "earnings_yield"],
          },
        ...
        },
        {
          ...
          title: "执行 30 组并行训练",
          description: "在固定数据集上搜索 LightGBM 与 Temporal Convolution 组合。",
          phase: "REPAIR",
          kind: "WRITE",
          ...
          arguments: {
            ...
            targetRevision: scenario.targetRevision,
            trialCount: 30,
            architectures: ["LightGBM", "TemporalConvolution"],
            mode: "ITERATION",
          },
        ...
        },
        {
          ...
          title: "执行收益与风险评估",
          description: "核验 IC、夏普、最大回撤和对抗验证结果。",
          phase: "REPAIR",
          kind: "QUALITY_GATE",
    ...
    verificationCalls: [
      ...
      { toolName: "dataops.dataset.validation.get", arguments: { datasetUid: `dataset_quant_${scenario.targetRevision}` } },
      { toolName: "mlops.attribution.report.get", arguments: { deploymentUid: scenario.deploymentUid, reportUid: scenario.reportUid } },
      { toolName: "mlops.deployment.get", arguments: { deploymentUid: scenario.deploymentUid, includeRevisions: true } },
    ...
    skill: {
      ...
      workflow: ["校验 AIOps 服务健康、盘后数据和工作流完整性。", "完成因子贡献、市场漂移和模型退化归因。", "由 AgentTeams 形成固定因子变更与完整迭代计划并提交审批。", "构建五年训练数据集并发布候选特征流水线。", "执行 30 组并行训练、效果与风险评估并登记模型卡。", "在模拟盘执行 1% 灰度和全量信号提升。", "独立验证数据集、归因门槛、目标修订、发布状态与推理探针。"],
